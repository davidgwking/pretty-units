...
function prettyUnits(n, exp) {
  ...
  if (abs <= prefixes[0].pow10) bestMatchIndex = 0;
  if (abs >= prefixes[prefixes.length - 1].pow10) bestMatchIndex = prefixes.length - 1;
  ...
  // construct output
  var newNum = round(number / prefixes[bestMatchIndex].pow10, 2);
